          ...
          <div>
            ...
            <div v-if="form.password" class="mt-1.5 flex gap-1">
              <div v-for="i in 4" :key="i"
                ...
                :class="i <= pwStrength ? pwStrengthColor : 'bg-[var(--color-border)]'">
              </div>
            </div>
...
const emailRe  = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
// Phone entered without +880 prefix: must be 10 digits starting with 1[3-9]
const phoneRe  = /^1[3-9]\d{8}$/
const strongPw = /^(?=.*[A-Za-z])(?=.*\d).{8,}$/

// ── Server-side field errors (e.g. duplicate email/phone) ────────────────────
const serverFieldErrors = ref<Record<string, string>>({})

// Merge server field errors into the computed errors
const errors = computed(() => {
  ...
  if (touched.value.division && !form.value.division)
    e.division = 'Please select your division'
...
async function register() {
  ...
  try {
    ...
    if (!res.ok) {
      // If the server tells us which field is duplicate, show it inline
