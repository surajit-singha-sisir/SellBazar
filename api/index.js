// api/index.js — Vercel Serverless Function (plain JS, no compilation needed)
// This is the SINGLE entry point Vercel runs for all /api/* requests.
...
if (!url || !token) {
  console.error('FATAL: Redis env vars missing. Set UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN in Vercel dashboard.')
}
...
const KEYS = {
  ...
  reviews:    'sb:reviews',
  banners:    'sb:banners',
  cart:    (uid) => `sb:cart:${uid}`,
  wishlist:(uid) => `sb:wishlist:${uid}`,
}
...
app.get('/api/health', (_req, res) => {
  res.json({ status: 'ok', service: 'SellBazar API', time: new Date().toISOString(), redisOk: !!redis })
})
...
// ── Seed data helpers ─────────────────────────────────────────────────────────
async function getWithSeed(key, seedFlagSuffix, seedData) {
  const seeded = await redis.get(`${KEYS.seeded}:${seedFlagSuffix}`)
  if (seeded) return (await redis.get(key)) ?? []
  await redis.set(key, seedData)
  await redis.set(`${KEYS.seeded}:${seedFlagSuffix}`, '1')
  return seedData
}
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (users.find(u => normalizePhone(u.phone) === normalizePhone(phone)))
      return res.status(409).json({ field:'phone', error:'This phone number is already registered' })
    const newUser = { id:Date.now().toString(), name:name.trim(), email:email.toLowerCase().trim(),
...
async function getProducts() { return getWithSeed(KEYS.products, 'products', SEED_PRODUCTS) }
async function saveProducts(p) { await redis.set(KEYS.products, p) }
...
app.put('/api/products/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const b = req.body, cur = products[idx]
    products[idx] = {
      ...cur,
...
app.get('/api/user/cart', async (req, res) => {
  try {
    ...
    res.json({ cart: (await redis.get(KEYS.cart(uid))) ?? [] })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.post('/api/user/cart', async (req, res) => {
  try {
    ...
    await redis.set(KEYS.cart(uid), cart); res.json({ ok:true })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.get('/api/user/wishlist', async (req, res) => {
  try {
    const uid = getUserId(req); if (!uid) return res.status(401).json({ error:'Unauthorized' })
    res.json({ wishlist: (await redis.get(KEYS.wishlist(uid))) ?? [] })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.post('/api/user/wishlist', async (req, res) => {
  try {
    ...
    await redis.set(KEYS.wishlist(uid), wishlist); res.json({ ok:true })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.delete('/api/user/userdata', async (req, res) => {
  try {
    const uid = getUserId(req); if (!uid) return res.status(401).json({ error:'Unauthorized' })
    await redis.del(KEYS.cart(uid)); await redis.del(KEYS.wishlist(uid)); res.json({ ok:true })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
async function getCategories() { return getWithSeed(KEYS.categories, 'categories', SEED_CATEGORIES) }
async function saveCategories(c) { await redis.set(KEYS.categories, c) }
...
app.post('/api/categories', requireAdmin, async (req, res) => {
  try {
    ...
    cats.push(newCat); await saveCategories(cats); res.status(201).json(newCat)
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.put('/api/categories/:slug', requireAdmin, async (req, res) => {
  try {
    ...
    await saveCategories(cats); res.json(cats[idx])
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.delete('/api/categories/:slug', requireAdmin, async (req, res) => {
  try {
    ...
    const idx = cats.findIndex(c => c.slug === req.params.slug)
    if (idx===-1) return res.status(404).json({ error:'Category not found' })
    const [removed] = cats.splice(idx,1); await saveCategories(cats)
...
app.post('/api/categories/:slug/subcategories', requireAdmin, async (req, res) => {
  try {
    ...
    cat.subcategories.push(newSub); await saveCategories(cats); res.status(201).json(newSub)
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.put('/api/categories/:slug/subcategories/:subSlug', requireAdmin, async (req, res) => {
  try {
    ...
    await saveCategories(cats); res.json(cat.subcategories[subIdx])
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.delete('/api/categories/:slug/subcategories/:subSlug', requireAdmin, async (req, res) => {
  try {
    ...
    await saveCategories(cats); res.json({ message:'Subcategory deleted', slug:removed.slug })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
// REVIEWS  /api/reviews/*
// ═══════════════════════════════════════════════════════════════════════════════
async function getReviews() { return (await redis.get(KEYS.reviews)) ?? [] }
async function saveReviews(r) { await redis.set(KEYS.reviews, r) }
...
app.get('/api/reviews/check', async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ canReview:false, reason:'Order not found' })
    const users = (await redis.get(KEYS.users)) ?? []
    const user = users.find(u => u.id===userId)
    const userEmail = user?.email??''
    const ownsOrder = order.userId===userId || (userEmail && order.customer?.email?.toLowerCase()===userEmail.toLowerCase())
    ...
    const reviews = await getReviews()
    const already = reviews.find(r => r.orderId===orderId && r.productId===productId && r.userId===userId)
    if (already) return res.json({ canReview:false, reason:'Already reviewed', reviewId:already.id })
...
app.post('/api/reviews', async (req, res) => {
  try {
    ...
    const reviews = await getReviews()
    if (reviews.find(r => r.orderId===orderId && r.productId===productId && r.userId===userId))
    ...
    reviews.push(review); await saveReviews(reviews)
...
app.post('/api/reviews/:id/helpful', async (req, res) => {
  try {
    ...
    await saveReviews(reviews); res.json({ helpful:reviews[idx].helpful })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.put('/api/reviews/:id', async (req, res) => {
  try {
    ...
    if (status==='approved'||status==='rejected'||rating!==undefined) await syncProductRating(reviews[idx].productSlug)
...
app.delete('/api/reviews/:id', async (req, res) => {
  try {
    ...
    await saveReviews(reviews); await syncProductRating(deleted.productSlug)
...
// BANNERS  /api/banners/*
// ═══════════════════════════════════════════════════════════════════════════════
async function getBanners() { return (await redis.get(KEYS.banners)) ?? [] }

app.get('/api/banners', async (_req, res) => {
  try { res.json([...(await getBanners())].sort((a,b)=>a.order-b.order)) }
  catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.post('/api/banners', requireAdmin, async (req, res) => {
  try {
    ...
    const newBanner = { id:id||randomBytes(8).toString('hex'), tag:tag??'', title:title.trim(),
    ...
    banners.push(newBanner); await redis.set(KEYS.banners, banners); res.status(201).json(newBanner)
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.put('/api/banners/reorder', requireAdmin, async (req, res) => {
  try {
    ...
    await redis.set(KEYS.banners, banners); res.json(banners)
  } catch(e) { res.status(500).json({ error:e.message }) }
})
app.put('/api/banners/:id', requireAdmin, async (req, res) => {
  try {
    ...
    await redis.set(KEYS.banners,banners); res.json(banners[idx])
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.delete('/api/banners/:id', requireAdmin, async (req, res) => {
  try {
    ...
    await redis.set(KEYS.banners,filtered); res.json({ success:true })
  } catch(e) { res.status(500).json({ error:e.message }) }
})
...
app.post('/api/admin/migrate-seed-flags', requireAdmin, async (_req, res) => {
  try {
    const results={}
    for (const col of ['products','orders','categories']) {
      const data = await redis.get(KEYS[col])
      if (data!==null) { await redis.set(`${KEYS.seeded}:${col}`,'1'); results[col]=`flag set` }
      else results[col]='no data'
    }
...
app.delete('/api/admin/reset-collection/:col', requireAdmin, async (req, res) => {
  try {
    ...
    if (!['products','orders','categories'].includes(col)) return res.status(400).json({ error:'col must be products|orders|categories' })
    await redis.del(KEYS[col]); await redis.del(`${KEYS.seeded}:${col}`)
    res.json({ ok:true, message:`${col} cleared — will auto-reseed on next request` })
  } catch(e) { res.status(500).json({ error:e.message }) }
})

// ═══════════════════════════════════════════════════════════════════════════════
// SSE EVENTS  /api/events  (no-op on Vercel — serverless can't hold connections)
// ═══════════════════════════════════════════════════════════════════════════════
app.get('/api/events', (req, res) => {
  ...
  res.flushHeaders()
  // On Vercel serverless, long-lived connections aren't supported.
  // Send an immediate snapshot and close gracefully.
  Promise.all([redis.get(KEYS.orders), redis.get(KEYS.products)]).then(([orders, products]) => {
    try {
