...
router.get('/dashboard', requireAdmin, (req, res) => {
    ...
    const revenueByStatus = ['pending', 'processing', 'shipped', 'delivered', 'cancelled'].map(status => ({
        status,
        total: orders.filter((o) => o.status === status).reduce((s, o) => s + o.total, 0),
...
router.get('/customers', requireAdmin, (req, res) => {
    ...
    orders.forEach((o) => {
        ...
        if (o.createdAt > map[email].lastOrder)
            map[email].lastOrder = o.createdAt;
        if (o.createdAt < map[email].firstOrder)
            map[email].firstOrder = o.createdAt;
