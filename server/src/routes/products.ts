...
// ── Sort helper — newest updatedAt first, fall back to createdAt ──────────────
function sortByLatest(arr: any[]): any[] {
  return [...arr].sort((a, b) => {
...
router.get('/', async (req, res) => {
  ...
  if (sortBy) {
    // Explicit sort requested by the caller
    const dir = order === 'asc' ? 1 : -1
    result.sort((a: any, b: any) => a[sortBy] < b[sortBy] ? -1 * dir : a[sortBy] > b[sortBy] ? 1 * dir : 0)
  } else {
    // Default: newest updatedAt / createdAt first
...
router.put('/:id', requireAdmin, async (req: any, res) => {
  ...
  const updated: any = {
    ...
    updatedAt: new Date().toISOString(),  // always bump on every edit
  }
